...
Its a really simple two view Controllers code. The first calls the itunes search api to find list of games with keyword *fifa* and the next view controller just loads the game image.
...
1. **Toggle Breakpoints** This is a easy switch to disable all breakpoints/enable them at once.  
2. **Play/Pause** This button lets you pause your code at a certain time and once you are ready to continue, just press it again to continue execution.  
3. **Step Over** Step over is the most important and most used tool in debugging for me. What it does is process the current line of code and then stop the thread at the next line, so essentially you use it to execute and debug your code line by line.
...
So how to proceed ?
The error points to a mutating member on a immutable object. If you already know the error and its fix, just go ahead and fix it yourself. However, in case if you dont know how to fix, its time for you to Google!.
...
**Thank you** for reading this far. Hopefully you would have learned some helpful tips from this tutorial and are a debug ninja now.
